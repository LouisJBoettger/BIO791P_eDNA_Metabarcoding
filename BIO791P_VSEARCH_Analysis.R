...
extract_species <- function(x) {
  ...
  species <- taxa[
    str_detect(taxa, "^[A-Za-z]+_[A-Za-z]+$")]
  unique(species)
}
...
resolve_otu <- function(df) {
  ...
  best <- df |> 
    filter(Percent_identity == best_id)
  
  genera <- unique(
    unlist(lapply(best$Taxon_raw,extract_genus)))
  ...
  if (length(species) > 1) {
    ...
    return(
      tibble(Genus = genus, Species = NA_character_,
...
vsearch_all <- vsearch_all |> 
  left_join(otu_taxonomy, by = "OTU_ID")


# table for OTUs
otu_table <- vsearch_all |> 
  distinct(Barcode, OTU_ID, .keep_all = TRUE)
...
eDNA_genus <- otu_soil |>
  filter(Assignment_level %in% c("Species", "Genus")) |>
...
ggplot(
  ...
  geom_point(
    size = 3) +
  theme_bw() +
  labs(x = paste0("PCoA1 (",species_abundance_variance[1],"%)"),
...
species_pa_dist <- vegdist(species_pa_matrix,method = "bray")

species_pa_pcoa <- cmdscale(species_pa_dist, eig = TRUE, k = 2)

species_pa_variance <- round(
  species_pa_pcoa$eig /
    sum(species_pa_pcoa$eig[species_pa_pcoa$eig > 0]) *100, 1)

species_pa_scores <- data.frame(
  Barcode = rownames(species_pa_pcoa$points),
...
ggplot(
  ...
  geom_point(
    size = 3) +
  theme_bw() +
  labs(x = paste0("PCoA1 (",species_pa_variance[1],"%)"),
...
genus_abundance_dist <- vegdist(genus_abundance_matrix,method = "bray")

genus_abundance_pcoa <- cmdscale(genus_abundance_dist, eig = TRUE, k = 2)
...
genus_abundance_scores <- data.frame(
  Barcode = rownames(genus_abundance_pcoa$points),
...
ggplot(
  ...
  geom_point(size = 3) +
  theme_bw() +
  labs(x = paste0("PCoA1 (",genus_abundance_variance[1],"%)"),
...
genus_pa_dist <- vegdist(genus_pa_matrix, method = "bray")

genus_pa_pcoa <- cmdscale(genus_pa_dist, eig = TRUE, k = 2)
...
ggplot(eDNA_location_summary, aes(x = eDNA_Shannon)) +
  ...
  labs(x = "eDNA Shannon Diversity Index", y = "Number of locations")
...
eDNA_diversity_location_A9 <- eDNA_diversity_location |>
  left_join(
    genus_pa_scores |> select(Barcode, Dist_A9), by = "Barcode")
